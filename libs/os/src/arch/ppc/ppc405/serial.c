...
static os_err_t rt_serial_open(device_t dev, uint16_t oflag)
{
    ...
    if (dev->flag & RT_DEVICE_FLAG_INT_RX) {
        ...
        out_8((uint8_t*)device->hw_base + UART_IER, 0x01);
        /* Setup UART FIFO: RX trigger level: 1 byte, Enable FIFO */
        out_8((uint8_t*)device->hw_base + UART_FCR, 1);

        /* init UART rx interrupt */
        os_arch_interrupt_unmask(device->irqno);
    }
...
static os_err_t rt_serial_close(device_t dev)
{
    ...
    if (dev->flag & RT_DEVICE_FLAG_INT_RX) {
        ...
        os_arch_interrupt_mask(device->irqno);
    }
...
static size_t rt_serial_write(device_t dev, offset_t pos, const void* buffer, size_t size)
{
    ...
    if (dev->flag & RT_DEVICE_FLAG_STREAM) {
        ...
        while (size) {
            if (*ptr == '\n') {
                while ((in_8((uint8_t*)device->hw_base + UART_LSR) & 0x20) != 0x20);
                out_8((uint8_t*)device->hw_base + UART_THR, '\r');
            }

            while ((in_8((uint8_t*)device->hw_base + UART_LSR) & 0x20) != 0x20);
            out_8((uint8_t*)device->hw_base + UART_THR, *ptr);
...
void rt_serial_set_baudrate(struct rt_ppc405_serial* device)
{
    ...
    bdiv = 115200;
    out_8((uint8_t *)device->hw_base + UART_DLL, bdiv);        /* set baudrate divisor */
    out_8((uint8_t *)device->hw_base + UART_DLM, bdiv >> 8); /* set baudrate divisor */
}
...
void rt_serial_isr(int irqno, void* param)
{
    ...
    status = in_8((uint8_t *)device->hw_base + UART_LSR);
    ...
    if (status & 0x01) {
        ...
        while (status & 0x01) {
            ...
            /* read character */
            device->rx_buffer[device->save_index] = (0xff & (int) in_8((uint8_t *)device->hw_base));
            device->save_index++;
            ...
            if ((status & (asyncLSRFramingError1 |
                ...
                out_8((uint8_t *)device->hw_base + UART_LSR,
        ...
        /* invoke callback */
        if (device->parent.rx_indicate != NULL) {
          device->parent.rx_indicate(&device->parent, 1);
        }
    }
}

void os_arch_serial_init(void)
{
    ...
    os_arch_interrupt_install(device->irqno, rt_serial_isr, device, "serial"); /* install isr */

    os_memset(device->rx_buffer, 0, sizeof(device->rx_buffer));
    device->read_index = device->save_index = 0;

    out_8((uint8_t *)device->hw_base + UART_LCR, 0x80);    /* set DLAB bit */
    /* setup baudrate */
    rt_serial_set_baudrate(device);
    out_8((uint8_t *)device->hw_base + UART_LCR, 0x03);    /* clear DLAB; set 8 bits, no parity */
    out_8((uint8_t *)device->hw_base + UART_FCR, 0x00);    /* disable FIFO */
    out_8((uint8_t *)device->hw_base + UART_MCR, 0x00);    /* no modem control DTR RTS */
    val = in_8((uint8_t *)device->hw_base + UART_LSR);    /* clear line status */
    val = in_8((uint8_t *)device->hw_base + UART_RBR);    /* read receive buffer */
    out_8((uint8_t *)device->hw_base + UART_SCR, 0x00);    /* set scratchpad */
    out_8((uint8_t *)device->hw_base + UART_IER, 0x00);    /* set interrupt enable reg */
