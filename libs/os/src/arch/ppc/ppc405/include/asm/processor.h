...
struct os_arch_type *identify_cpu(u32 ver);
...
struct task_struct;
void start_task(struct pt_regs *regs, uint32_t nip, uint32_t sp);
void release_task(struct task_struct *);

/*
 * Create a new kernel task.
 */
extern long kernel_task(int (*fn)(void *), void *arg, uint32_t flags);
...
struct thread_struct {
    ...
    uint32_t    wchan;        /* Event task is sleeping on */
    struct pt_regs    *regs;        /* Pointer to saved register state */
    mm_segment_t    fs;        /* for get_fs() validation */
    void        *pgdir;        /* root of page-table tree */
    signed long    last_syscall;
    double        fpr[32];    /* Complete floating point set */
    uint32_t    fpscr_pad;    /* fpr ... fpscr must be contiguous */
    uint32_t    fpscr;        /* Floating point status */
...
#define INIT_SP        (sizeof(init_stack) + (uint32_t) &init_stack)
...
/*
 * Return saved PC of a blocked task. For now, this is the "user" PC
 */
static inline uint32_t thread_saved_pc(struct thread_struct *t)
{
...
uint32_t get_wchan(struct task_struct *p);

#define KSTK_EIP(tsk)  ((tsk)->task.regs->nip)
#define KSTK_ESP(tsk)  ((tsk)->task.regs->gpr[1])
...
#define free_task_struct(p)    free_pages((uint32_t)(p),1)
...
/* In misc.c */
void _nmask_and_or_msr(uint32_t nmask, uint32_t or_val);
