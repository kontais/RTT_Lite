...
#define THREAD_T(task)  ((thread_t *)task)
...
static volatile unsigned int  os_arch_pending_interrupts;
static int (* os_arch_isr_table[MAX_INTERRUPT_NUM])(void) = {0};
...
static void thread_suspend_signal_handler(int sig)
{
    ...
    thread_from = (thread_t *) interrupt_switch_task_from;
    thread_to = (thread_t *) interrupt_switch_task_to;

    /* 注意！此时 os_task_self的值是to线程的值！ */
    tid = os_task_self();
    /* FIXME OS_ASSERT(thread_from->pthread == pid); */
    OS_ASSERT((thread_t *)(tid->sp) == thread_to);
    ...
    thread_to = (thread_t *) interrupt_switch_task_to;
    OS_ASSERT(thread_to == thread_from);
...
static void *thread_run(void *parameter)
{
    os_task_t tid;
    thread_t *task;
    task = THREAD_T(parameter);
    int res;

    /* set signal mask, mask the timer! */
    signal_mask();

    task->status = SUSPEND_LOCK;
    TRACE("pid <%08x> stop on sem...\n", (unsigned int)(task->pthread));
    sem_wait(&task->sem);
...
static int thread_create(
    thread_t *task, void *task, void *parameter, void *pexit)
{
    int res;
    pthread_attr_t attr;
    ...
    if (sem_init(&task->sem, 0, 0) != 0) {
        printf("init task->sem failed, exit \n");
        exit(EXIT_FAILURE);
    }
    /* No need to join the tasks. */
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_deleteED);

    /* create a posix task */
    res = pthread_create(&task->pthread, &attr, &thread_run, (void *)task);
    if (res) {
...
/* resume the task */
static int thread_resume(thread_t *task)
{
    sem_post(& task->sem);
}

uint8_t *os_arch_stack_init(
    void *pEntry,
    void *pParam,
    uint8_t *pStackAddr,
    void *pExit)
{
    ...
    task = (thread_t *)(pStackAddr - sizeof(thread_t));

    /* set the filed to zero */
    memset(task, 0x00, sizeof(thread_t));

    thread_create(task, pEntry, pParam, pExit);
    //TRACE("task %x created\n", (unsigned int)thread_table[t].pthread);
...
void os_arch_context_switch_interrupt(uint32_t from,
                                    uint32_t to)
{
    os_arch_context_switch(from, to);
}
...
void os_arch_context_switch_to(uint32_t to)
{
    ...
    //set interrupt to 1
    interrupt_switch_flag = 0; //TODO: 还需要考虑这个嵌套切换的情况

    /* enable interrupt
     * note: NOW, there are only one interrupt in simposix: system tick */
    os_exit_critical(0);

    //start the main task scheduler
...
static int mainthread_scheduler(void)
{
    ...
    /* install signal handler used to suspend/resume tasks */
...
static void start_sys_timer(void)
{
    ...
    OS_ASSERT(OS_TICKS_PER_SEC <= 1000000 || OS_TICKS_PER_SEC >= 1);

    us = 1000000 / OS_TICKS_PER_SEC - 1;
