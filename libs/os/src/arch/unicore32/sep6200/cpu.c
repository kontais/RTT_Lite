...
rt_inline uint32_t icache_status(void)
{
    uint32_t ret;
...
rt_inline uint32_t dcache_status(void)
{
    uint32_t ret;
...
static void sep6200_reset(uint32_t addr)
{
     __asm__ volatile ( "mov ip, #0\n"
...
{
    printk("sep6200 power off not implemented\n");
  while (1);
}
...
void os_arch_reset()
{
    ...
    printk("Soft reset, Restarting system...\n");
    sep6200_reset(0);
