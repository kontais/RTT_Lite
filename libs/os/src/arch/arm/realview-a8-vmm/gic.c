...
int arm_gic_get_active_irq(uint32_t index)
{
    ...
    OS_ASSERT(index < ARM_GIC_MAX_NR);

    irq = GIC_CPU_INTACK(_gic_table[index].os_arch_hw_base);
...
void arm_gic_ack(uint32_t index, int irq)
{
    ...
    GIC_CPU_EOI(_gic_table[index].os_arch_hw_base) = irq;
...
void arm_gic_clear_pending(uint32_t index, int irq)
{
    uint32_t mask = 1 << (irq % 32);
...
void arm_gic_dump(uint32_t index)
{
    ...
    k = GIC_CPU_HIGHPRI(_gic_table[index].os_arch_hw_base);
...
int arm_gic_dist_init(uint32_t index, uint32_t dist_base, int irq_start)
{
    ...
    if ((rt_os_arch_get_smp_id() & 0xF) != 0)
        return 0;
#endif
#ifdef RT_USING_VMM
    return 0;
...
int arm_gic_os_arch_init(uint32_t index, uint32_t os_arch_base)
{
    ...
    _gic_table[index].os_arch_hw_base = os_arch_base;
    ...
    GIC_CPU_PRIMASK(os_arch_base) = 0xf0;
    /* Enable CPU interrupt */
    GIC_CPU_CTRL(os_arch_base) = 0x01;
