...
void mmu_create_pgd(struct mem_desc *mdesc)
{
    ...
    volatile int i, nSec;
    pTT = (uint32_t *)_pgd_table + (mdesc->vaddr_start >> 20);
...
{
    volatile uint32_t *pTT;
    volatile uint32_t *p_pteentry;
    int i;
    uint32_t vaddr;
    uint32_t total_page = 0;
    uint32_t pte_offset = 0;
    uint32_t sect_attr = 0;
...
static void build_pte_mem_desc(struct mem_desc *mdesc, uint32_t size)
{
    uint32_t pte_offset = 0;
    uint32_t nsec = 0;
    /* set page table */
    for (; size > 0; size--) {
        if (mdesc->mapped_mode == PAGE_MAPPED) {
            nsec = (OS_ALIGN(mdesc->vaddr_end, 0x100000) - OS_ALIGN_DOWN(mdesc->vaddr_start, 0x100000)) >> 20;
            mdesc->sect_attr |= (((uint32_t)_pte_table)& 0xfffffc00) + pte_offset;
            pte_offset += nsec << 10;
        }
        if (pte_offset >= RT_MMU_PTE_SIZE) {
            printk("PTE table size too little\n");
            OS_ASSERT(0);
        }
...
void os_arch_mmu_init(struct mem_desc *mdesc, uint32_t size)
{
    ...
    os_memset((void *)_pgd_table, 0, 16*1024);
    os_memset((void *)_pte_table, 0, RT_MMU_PTE_SIZE);
    ...
    mmu_setttbase((uint32_t)_pgd_table);
