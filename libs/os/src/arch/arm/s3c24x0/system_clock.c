...
void os_arch_set_mpll_clock(uint8_t sdiv, uint8_t pdiv, uint8_t mdiv)
{
    MPLLCON = sdiv | (pdiv<<4) | (mdiv<<12);
}

void os_arch_set_upll_clock(uint8_t sdiv, uint8_t pdiv, uint8_t mdiv)
{
    UPLLCON = (mdiv<<12) | (pdiv<<4) | sdiv;
}

void os_arch_set_divider(uint8_t hdivn, uint8_t pdivn)
{
...
{
    LOCKTIME = 0xFFFFFFFF;
    os_arch_set_mpll_clock(MPL_SDIV, MPL_PDIV, MPL_MIDV);
    os_arch_set_upll_clock(UPL_SDIV, UPL_PDIV, UPL_MDIV);
    os_arch_set_divider(HDIVN, PDIVN);
}
