...
#ifndef os_arch_dcache_line_size
#define os_arch_dcache_line_size()    g_mips_core.icache_line_size
#endif

#ifndef os_arch_icache_line_size
#define os_arch_icache_line_size()    g_mips_core.icache_line_size
#endif
...
static inline void flush_icache_line_indexed(uint32_t addr)
{
    cache_op(Index_Invalidate_I, addr);
}

static inline void flush_dcache_line_indexed(uint32_t addr)
{
    cache_op(Index_Writeback_Inv_D, addr);
}

static inline void flush_icache_line(uint32_t addr)
{
    cache_op(Hit_Invalidate_I, addr);
}

static inline void lock_icache_line(uint32_t addr)
{
    cache_op(Fetch_And_Lock_I, addr);
}

static inline void lock_dcache_line(uint32_t addr)
{
    cache_op(Fetch_And_Lock_D, addr);
}

static inline void flush_dcache_line(uint32_t addr)
{
    cache_op(Hit_Writeback_Inv_D, addr);
}

static inline void invalidate_dcache_line(uint32_t addr)
{
...
{
    uint32_t start = KSEG0;
    uint32_t end = start + dcache_size;
    uint32_t addr;
...
{
    uint32_t start = KSEG0;
    uint32_t end = start + icache_size;
    uint32_t addr;
