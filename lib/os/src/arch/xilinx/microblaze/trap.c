...
void os_arch_interrupt_init()
{
    ...
    for (index = 0; index < MAX_HANDLERS; index++) {
        CfgPtr->HandlerTable[index].Handler = (XInterruptHandler)os_arch_interrupt_handler;
    }
...
void os_arch_interrupt_install(int vector, rt_isr_handler_t new_handler, rt_isr_handler_t *old_handler)
{
    ...
    if (vector >= 0 && vector < MAX_HANDLERS) {
        if (*old_handler != NULL) *old_handler = (rt_isr_handler_t)CfgPtr->HandlerTable[vector].Handler;
        if (new_handler != NULL) CfgPtr->HandlerTable[vector].Handler = (XInterruptHandler)new_handler;
    }
}
